<template>
  <div class="m-detailwrap">
    ...
    <div class="content-list">
      <ul class="state-list">
        <li v-for="(item, index) in states" :class="{stateactive: index == stateactived}" @click="filterBtn(index)">{{item}}
        </li>
      </ul>
    </div>
    <div class="m-list f-cb">
      ...
      <div class="m-child f-fl" v-for="(item, index) in tableData" @mouseenter="maskShow(index)" @mouseleave="maskFade(index)">
        <div class="m-img">
          ...
          <div class="tag" :class="{enroll:item.state=='报名中',check:item.state=='待审核',
          ...
          <transition name='fade'>
            <div class="fade-mask" v-if="index == listIndex" @click="skipToManage(item.id)">
        ...
        <div class="info f-cb">
          ...
          <a class="btn f-fl" href="javascript:void(0)" v-if="item.state!='草稿'" @click="skipToManage(item.id)">管理</a>
          ...
          <el-dialog :visible.sync="dialogFormVisible" size="tinyX">
            ...
            <div class="btn-wrap">
              <button class="confirm btn" @click="confirm(item.id)">确定</button>
            </div>
          </el-dialog>
        </div>
      </div>
...
export default {
  data() {
    return {
      tableData: [],
      types: ['已发布', '已取消', '草稿'],
      states: ['全部', '待审核', '预备中', '报名中', '预热中', '进行中', '已结束'],
      actived: 0,
      stateactived: 0,
      filterdata: {},
      total: 0,
  ...
  methods: {
    ...
    filterBtn(index) {
      ...
      if (index == 0) {
        this.filterdata.states = ["1", "3", "4", "5", "6", "7"];
        this.getActivityList(this.filterdata);
      } else if (index == 1) {
        this.filterdata.states = ["1"];
        this.getActivityList(this.filterdata);
      } else {
        this.filterdata.states = [index + 1];
        this.getActivityList(this.filterdata);
      }
    },
    maskShow(index) {
