	...
	<div class="activitycreate">
		<div class="g-section">
			<p class="m-title f-cb">
				...
				<span class="name f-db f-fl">发布活动</span>
			</p>
			<div class="m-detailwrap">
				<el-form ref="form" :model="form" :rules="rules" label-width="193px" label="right" class="publishForm">
					...
					<el-form-item label="活动海报" prop="coverImg" required style="height:386px;">
						<el-row>
							...
							<div class="upload f-cb">
								...
								<a href="javascript:void(0)" class="poster">没有准备海报?可选择
									<span class="hfcl" @click="imageDialogVisible = true">精选海报</span>
								</a>
							</div>
						...
						<el-dialog title="选择一张图片作为海报：" v-model="imageDialogVisible" size="small" style="min-width:672px;">
							...
							<span slot="footer" class="dialog-footer">
								<el-button type="primary" @click="surePoster" class="hbtn">确 定</el-button>
								<el-button @click="imageDialogVisible = false" class="hcbtn">取消</el-button>
							</span>
						</el-dialog>
					</el-form-item>
					<el-form-item label="活动人数" prop="num" required id="activityNum">
						<el-col :span="4">
							<el-input class="m-num" v-model="form.num" :disabled="form.numLimit"></el-input>
						</el-col>
						<el-col :span="3">
						...
						<el-col :span="18" class="guest-info m-left">
							...
							<el-row>
								<el-col :span="23" class="s-textarea">
									<el-input type="textarea" :rows="8" v-model="form.guestInfo" placeholder="请填写嘉宾简介"></el-input>
								</el-col>
							</el-row>
						</el-col>
					...
					<el-form-item v-for="(guest, index) in guestLists" :key="guest">
						<div class="guestLists f-cb">
							<div class="guestimg f-fl">
								<img :src="guest.pic?guest.pic:'http://huicye.image.alimmdn.com/FED/huicye-dreamtown/body-icon.png'" />
							</div>
					...
					<el-form-item label="活动流程" style="height:36px;">
		...
		onSubmit(e) {
			...
			if (option == '立即发布') {
					...
					if (valid) {
						...
						axios.post(this.uploadUrl + '?action=uploadHTML', formData).then(res => {
							...
							activityApi.activity.postCreate(postdata).then(res => {
								...
								setTimeout(() => {
									window.location.pathname = '/nav1/active/activitycreate/success'
								}, 1000)
							})
						})
					} else {
				...
				axios.post(this.uploadUrl + '?action=uploadHTML', formData).then(res => {
					...
					this.$router.push({ path: '/nav1/active/activitycreate/preview' });
				})
			}
		},
		getActivityPreview() {
			...
			if (localStorage.getItem('previewdata')) {
				...
				console.log(previewData,111)
				this.form.title = previewData.title;
				this.form.location = [previewData.location.province, previewData.location.city];
				this.form.address = previewData.location.area;
				if (previewData.startOfActivity) {
					this.form.activityStartTimeDate = functionApi.moment.toGMTFormate(functionApi.moment.getDateFormate(previewData.startOfActivity));
					this.form.activityStartTimeTime = functionApi.moment.toGMTFormate('2017-05-23' + ' ' + functionApi.moment.getHMTime(previewData.startOfActivity));
				} else {
					this.form.activityStartTimeDate = '';
					this.form.activityStartTimeTime = '';
				}

				if (previewData.endOfActivity) {
					this.form.activityEndTimeDate = functionApi.moment.toGMTFormate(functionApi.moment.getDateFormate(previewData.endOfActivity));
					this.form.activityEndTimeTime = functionApi.moment.toGMTFormate('2017-05-23' + ' ' + functionApi.moment.getHMTime(previewData.endOfActivity));
				} else {
					this.form.activityEndTimeDate = '';
					this.form.activityEndTimeTime = '';
				}


				if (previewData.endOfEnrollment) {
					this.form.setEnrollEndTime = true;
				}
				this.form.enrollEndTimeDate = functionApi.moment.toGMTFormate(functionApi.moment.getDateFormate(previewData.endOfEnrollment));
				this.form.enrollEndTimeTime = functionApi.moment.toGMTFormate('2017-05-23' + ' ' + functionApi.moment.getHMTime(previewData.endOfEnrollment));
				this.form.coverImg = previewData.cover;
				if (previewData.numberOfExpected == -1) {
					this.form.numLimit = true;
				} else {
					this.form.num = previewData.numberOfExpected;
				}
				this.form.subject = previewData.subject;
				this.form.pattern = previewData.pattern;
				this.form.sponsorTags = previewData.sponsor;
				this.form.coSponsorTags = previewData.coSponsor;
				console.log(previewData.content,11)
				$('#activityContent').load(previewData.content + " article", function () {
					console.log(previewData.content,11)
					console.log($('article'))
				...
				setTimeout(() => {
					this.guestLists = previewData.guests ? previewData.guests : [];
					if (previewData.process) {
						this.processLists = previewData.process;
					} else {
						this.processLists = [];
					}
				}
					, 1000);
				//this.guestLists = previewData.guests ? previewData.guests : [];
				this.form.contact = previewData.contact;
				this.form.phonenumber = previewData.phonenumber;

				if (previewData.activityForm) {
					this.enrollform = [
						{
							name: 'name',
							label: '姓名',
							need: previewData.activityForm.name.need == '1' ? true : false,
							require: previewData.activityForm.name.require == '1' ? true : false,
							placeholder: previewData.activityForm.name.placeholder
						},
						{
							name: 'phone',
							label: '电话',
							need: previewData.activityForm.phone.need == '1' ? true : false,
							require: previewData.activityForm.phone.require == '1' ? true : false,
							placeholder: previewData.activityForm.phone.placeholder
						},
						{
							name: 'company',
							label: '公司',
							need: previewData.activityForm.company.need == '1' ? true : false,
							require: previewData.activityForm.company.require == '1' ? true : false,
							placeholder: previewData.activityForm.company.placeholder
						}
					]
				}
				this.form.cost = previewData.cost;
				if (previewData.privacy == 1) {
