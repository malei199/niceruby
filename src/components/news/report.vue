    ...
    <div class="main">
        ...
        <div class="content main-con">
            <p class="activityInfo">
                <span>文章标题：</span>{{newsDetail.title}}</p>
            <p class="activityInfo">
                <span>文章作者：</span>{{newsDetail.author.name}}</p>
            <p class="activityInfo">
                <span>文章类型：</span>{{newsDetail.type}}</p>
            <p class="activityInfo">
                <span>发布时间：</span>{{newsDetail.update}}</p>
        </div>
        <div class="c-cl main-con">
            <span></span>举报人信息</div>
        ...
        <div class="content main-con reportson">
            <textarea class="report-reason" v-model="form.content" placeholder="举报原因必须包括要求删除稿件原因的详细说明，如：恶意诽谤请说明真实情况；造成不良影响请说明造成什么样的影响或可能的后果等。"></textarea>
...
export default {
    ...
    methods: {
        submit() {
            const dreamuser = localStorage.getItem('huicyeuser');
            if (!dreamuser) {
                alert("先请登录");
                ...
                this.newsId = location.pathname.split('/')[2];
                this.form.newsId = this.newsId;
                Api.news.postReport(this.form).then(res => {
                    this.$router.push({ path: '/newsReportSuccess/' + res.data.data.reportId });
                })
            }
        }
    },
    mounted() {
        ...
        Api.news.postDetail({ id: this.newsId }).then(res => {
            this.newsDetail = res.data.data;
            this.newsDetail.type = this.newsType[this.newsDetail.type];
            this.newsDetail.update = functionApi.moment.getListTime(this.newsDetail.publish);
        });
    }
}
</script>
